...
class ClientCertCapableSSLAdapter(wsgiserver.SSLAdapter):
    ...
    def __init__(self, certificate, private_key, certificate_chain=None,
                 client_CA=None, client_check='ignore', check_host=False):
        if ssl is None:
        ...
        if check == "ignore":
            ...
            raise ValueError("client_check must be one of 'ignore',"
                             "'optional','required'")

    def decode_cert(self, prefix, cert):
        if not cert:
            return None
...
##            SSL_VERSION_LIBRARY 	string 	The OpenSSL program version
        ...
        if self.client_CA and (client_cert or self.check == ssl.CERT_REQUIRED):
            ...
            client_cert_subject = self.decode_cert( "SSL_CLIENT_S_DN", client_cert["subject"] )
            if self.check_host:
                try:
